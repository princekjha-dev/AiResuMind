...
class ResumeAnalyzer:
    def __init__(self):
        ...
        self.document_types = {
            'resume': [
                'resume', 'cv', 'curriculum', 'vitae', 'contact', 'profile', 'experience', 
                'education', 'skills', 'work', 'project', 'projects', 'objective', 'summary', 
                'employment', 'qualification', 'achievements', 'certifications', 'languages', 
                'personal', 'b.tech', 'b.e', 'm.tech', 'bca', 'mca', 'bsc', 'msc', 'bba', 
                'mba', 'developer', 'engineer', 'analyst', 'manager', 'lead', 'intern', 
                'internship', 'phone', 'email', 'linkedin', 'github'
            ],
            'marksheet': [
    ...
    def detect_document_type(self, text):
        ...
        # Check if non-resume document type strongly matches
        if best_match[0] != 'resume' and best_match[1] > 0.25:
            return best_match[0]
    ...
    def calculate_keyword_match(self, resume_text, required_skills):
        ...
        for skill in required_skills:
            ...
            if skill_lower in resume_text or any(skill_lower in phrase for phrase in resume_text.split('.')):
    ...
    def extract_text_from_pdf(self, file):
        try:
            ...
            raise Exception(f"Error extracting text from PDF: {e!s}")
    ...
    def extract_text_from_docx(self, docx_file):
        ...
        try:
            ...
            raise Exception(f"Error extracting text from DOCX file: {e!s}")
    ...
    def analyze_resume(self, resume_data, job_requirements):
        ...
        try:
            ...
            ats_score = (
                round(contact_score * 0.1) +      # 10% weight for contact info
                round(summary_score * 0.1) +      # 10% weight for summary
                round(skills_score * 0.3) +       # 30% weight for skills match
                round(experience_score * 0.2) +   # 20% weight for experience
                round(education_score * 0.1) +    # 10% weight for education
                round(format_score * 0.2)         # 20% weight for formatting
            )
            ...
            import traceback
            print(f"Error analyzing resume: {e!s}")
            print(traceback.format_exc())
            # Return a default error response
            return {
                'error': f"Resume analysis failed: {e!s}",
                'ats_score': 0,
                'document_type': 'unknown',
                'keyword_match': {'score': 0, 'found_skills': [], 'missing_skills': []},
                'section_score': 0,
                'format_score': 0,
                'suggestions': [f"Error analyzing resume: {e!s}. Please check your file and try again."]
            }