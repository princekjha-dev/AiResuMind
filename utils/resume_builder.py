...
class ResumeBuilder:
    def __init__(self):
        self.templates = {
            ...
            "Traditional ATS": self.build_academic_template
        }
    ...
    def generate_resume(self, data):
        ...
        try:
            ...
            if 'academic' in template_name or 'traditional' in template_name:
                ...
                print(f"Warning: Unknown template '{template_name}', falling back to academic template")
                doc = self.build_academic_template(doc, data)
            ...
            print(f"Error in generate_resume: {e!s}")
    ...
    def build_modern_template(self, doc, data):
        ...
        try:
            ...
            print(f"Error in build_modern_template: {e!s}")
            raise
    ...
    def build_professional_template(self, doc, data):
        ...
        try:
            ...
            print(f"Error in build_professional_template: {e!s}")
            raise
    ...
    def build_minimal_template(self, doc, data):
        ...
        try:
            ...
            print(f"Error in build_minimal_template: {e!s}")
            raise
    ...
    def build_creative_template(self, doc, data):
        ...
        try:
            ...
            print(f"Error in build_creative_template: {e!s}")
            raise

    def build_academic_template(self, doc, data):
        """Build Classic One-Page Academic / Traditional ATS Single-Column Resume (Times New Roman, Serif, Minimalist, High-ATS)"""
        try:
            styles = doc.styles
            
            # Name Style - Traditional Center Serif
            name_style = styles.add_style('Academic Name', WD_STYLE_TYPE.PARAGRAPH) if 'Academic Name' not in styles else styles['Academic Name']
            ...
            # Contact Style - Single line centered
            contact_style = styles.add_style('Academic Contact', WD_STYLE_TYPE.PARAGRAPH) if 'Academic Contact' not in styles else styles['Academic Contact']
            contact_style.font.size = Pt(9.5)
            contact_style.font.name = 'Times New Roman'
            contact_style.font.color.rgb = RGBColor(30, 30, 30)
            contact_style.paragraph_format.space_after = Pt(8)
            contact_style.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER

            # Section Style - All Caps Serif with bottom border line
            section_style = styles.add_style('Academic Section', WD_STYLE_TYPE.PARAGRAPH) if 'Academic Section' not in styles else styles['Academic Section']
            section_style.font.size = Pt(12)
            section_style.font.bold = True
            section_style.font.all_caps = True
            ...
            # Normal text style
            normal_style = styles.add_style('Academic Normal', WD_STYLE_TYPE.PARAGRAPH) if 'Academic Normal' not in styles else styles['Academic Normal']
            normal_style.font.size = Pt(10)
            ...
            # Header Name
            name_p = doc.add_paragraph((personal.get('full_name') or 'CANDIDATE NAME').upper())
            name_p.style = name_style

            # Subtitle/Title if present
            if personal.get('title'):
                sub_p = doc.add_paragraph(personal.get('title'))
                sub_p.style = contact_style
                sub_p.paragraph_format.space_after = Pt(2)

            # Contact line
            c_parts = []
            if personal.get('email'): c_parts.append(personal['email'])
            if personal.get('phone'): c_parts.append(personal['phone'])
            if personal.get('location'): c_parts.append(personal['location'])
            if personal.get('linkedin'): c_parts.append(f"LinkedIn: {personal['linkedin']}")
            if personal.get('portfolio'): c_parts.append(f"GitHub/Portfolio: {personal['portfolio']}")
            
            if c_parts:
                cp = doc.add_paragraph(' | '.join(c_parts))
                cp.style = contact_style

            # Horizontal Line Divider helper
            def add_divider():
            ...
            # 1. EDUCATION SECTION (Top section for Academic / Traditional ATS format)
            if data.get('education'):
                sec = doc.add_paragraph('EDUCATION')
                sec.style = section_style
                add_divider()
                for edu in data['education']:
                    p = doc.add_paragraph()
                    p.style = normal_style
                    ...
                    date_str = f"Graduation: {edu.get('graduation_date', '')}"
                    p.add_run(f" | {date_str}")
                    
                    p2 = doc.add_paragraph()
                    p2.style = normal_style
                    p2.paragraph_format.left_indent = Inches(0.15)
                    deg_text = f"{edu.get('degree', '')} in {edu.get('field', '')}"
                    p2.add_run(deg_text).italic = True
                    if edu.get('gpa'):
                        p2.add_run(f" | GPA: {edu['gpa']}")

            # 2. EXPERIENCE SECTION
            exps = data.get('experience') or data.get('experiences')
            if exps:
                sec = doc.add_paragraph('PROFESSIONAL EXPERIENCE')
                sec.style = section_style
                add_divider()
                for exp in exps:
                    p = doc.add_paragraph()
                    p.style = normal_style
                    p.add_run(f"{exp.get('position', exp.get('title', ''))}").bold = True
                    if exp.get('company'):
                    ...
                    if exp.get('description'):
                        d_para = doc.add_paragraph(exp['description'])
                        d_para.style = normal_style
                        d_para.paragraph_format.left_indent = Inches(0.2)
                    
                    if exp.get('responsibilities'):
                        for resp in self._format_list_items(exp['responsibilities']):
                            bp = doc.add_paragraph()
                            bp.style = normal_style
                            bp.paragraph_format.left_indent = Inches(0.35)
                            bp.add_run('•  ' + resp)

            # 3. PROJECTS SECTION
            if data.get('projects'):
                sec = doc.add_paragraph('SELECTED PROJECTS')
                sec.style = section_style
                add_divider()
                for proj in data['projects']:
                    p = doc.add_paragraph()
                    ...
                    p.add_run(proj['name']).bold = True
                    if proj.get('technologies'):
                        p.add_run(f" ({proj['technologies']})").italic = True
                    if proj.get('link'):
                        p.add_run(f" | {proj['link']}")
                    
                    if proj.get('description'):
                        dp = doc.add_paragraph(proj['description'])
                        dp.style = normal_style
            ...
            # 4. SKILLS SECTION
            skills = data.get('skills') or data.get('skills_categories')
            if skills:
                sec = doc.add_paragraph('TECHNICAL SKILLS & COMPETENCIES')
                sec.style = section_style
                add_divider()
                
                def add_skill_line(cat_key, title):
                    val = skills.get(cat_key)
                    if val:
                        p = doc.add_paragraph()
                        p.style = normal_style
                        p.paragraph_format.left_indent = Inches(0.15)
                        p.add_run(f"{title}: ").bold = True
                        p.add_run(', '.join(self._format_list_items(val)))

                add_skill_line('technical', 'Technical Skills')
                add_skill_line('tools', 'Tools & Technologies')
                add_skill_line('languages', 'Languages')
                add_skill_line('soft', 'Soft Skills & Leadership')

            # 5. ACHIEVEMENTS & HONORS SECTION
            achievements = data.get('achievements') or data.get('honors')
            if achievements:
                sec = doc.add_paragraph('HONORS & ACHIEVEMENTS')
                sec.style = section_style
                add_divider()
                for ach in self._format_list_items(achievements):
                    bp = doc.add_paragraph()
                    bp.style = normal_style
                    bp.paragraph_format.left_indent = Inches(0.35)
                    bp.add_run('•  ' + ach)

            # Page Margins - Standard 0.5 Inch Strict One Page Academic Layout
            ...
            print(f"Error in build_academic_template: {e!s}")
            raise
